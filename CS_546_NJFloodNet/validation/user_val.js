...
const valid_username = (username) => {
    return valid_string(username) && username.length <= 15 && username.length >= 5 && /^[a-zA-Z0-9_]+$/.test(username)
}
...
const valid_password = (password) => {
    if (typeof password === 'string' && password.length >= 8) {
        ...
        const cases = [hasUpperCase, hasLowerCase, hasNumbers, hasNonAlphanumeric, hasSpecialCharacters]

        if (cases.every(value => { return value })){
            return true;
        } else {
...
const toExport = { valid_string, valid_username, valid_password, valid_email, valid_state }
