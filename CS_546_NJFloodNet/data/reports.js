...
const addReport = async (user_id, username, timestamp, location, reportText, reportImage) => {
    try {
        ...
        const reportTextOk = sensorVal.valid_string(reportText) && reportText.length <= 500
        if (!reportTextOk) throw new Error("Invalid report text. Must be a string with maximum length of 500 characters.");
...
const deleteReport = async (reportId) => {
    try {
        const reportData = await reports()
        const deleteInfo = await reportData.findOneAndDelete({ _id: new ObjectId(reportId) })
        return true
    } catch (e) {
        throw new Error(e)
    }
