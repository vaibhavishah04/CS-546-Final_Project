...
const updateMeasurement = async (measurementId, updateData) => {
    ...
    for (let sensor of all_sensors) {
        let measurement_idx = sensor.measurements.indexOf(m => m._id === measurementId)
        if (measurement_idx) {
            tgt_sensor = sensor
    ...
    // update measurement
    tgt_sensor.measurements[tgt_measurement] = Object.assign(tgt_sensor.measurements[tgt_measurement], updateData)
    let updated_sensor = await sensorData.updateSensor(tgt_sensor.id, tgt_sensor)
